...
/**
 * serialport api and jni get from http://code.google.com/p/android-serialport-api/
 *
 * @author benjaminwan
 */
public class ComAssistantActivity extends Activity {
            ...
            if (buttonView == toggleButtonCOMA) {
                if (isChecked) {
                    if (toggleButtonCOMB.isChecked() && SpinnerCOMA.getSelectedItemPosition() == SpinnerCOMB.getSelectedItemPosition()) {
                        ShowMessage("SerialPort" + SpinnerCOMA.getSelectedItem().toString() + " is opend");
                        buttonView.setChecked(false);
                ...
                if (isChecked) {
                    if (toggleButtonCOMA.isChecked() && SpinnerCOMB.getSelectedItemPosition() == SpinnerCOMA.getSelectedItemPosition()) {
                        ShowMessage("SerialPort" + SpinnerCOMB.getSelectedItem().toString() + " is opend");
                        buttonView.setChecked(false);
                ...
                if (isChecked) {
                    if (toggleButtonCOMA.isChecked() && SpinnerCOMC.getSelectedItemPosition() == SpinnerCOMA.getSelectedItemPosition()) {
                        ShowMessage("SerialPort" + SpinnerCOMC.getSelectedItem().toString() + " is opend");
                        buttonView.setChecked(false);
                ...
                if (isChecked) {
                    if (toggleButtonCOMA.isChecked() && SpinnerCOMD.getSelectedItemPosition() == SpinnerCOMA.getSelectedItemPosition()) {
                        ShowMessage("SerialPort" + SpinnerCOMD.getSelectedItem().toString() + " is opend");
                        buttonView.setChecked(false);
    ...
    private void DispRecData(ComBean ComRecData) {
        ...
        if ((iRecLines > 500) && (checkBoxAutoClear.isChecked()))//
        {
            editTextRecDisp.setText("");
    ...
    private void OpenComPort(SerialHelper ComPort) {
        try {
            ...
            ShowMessage("open failed:no read/write permission!");
        } catch (IOException e) {
            ShowMessage("open failed:unknown error!");
        } catch (InvalidParameterException e) {
            ShowMessage("open failed:params error!");
        }
    }
